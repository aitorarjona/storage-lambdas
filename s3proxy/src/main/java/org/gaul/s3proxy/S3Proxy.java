...
public final class S3Proxy {
    private static final Logger logger = LoggerFactory.getLogger(S3Proxy.class);
    ...
    S3Proxy(Builder builder) {
        checkArgument(builder.endpoint != null || builder.secureEndpoint != null, "Must provide endpoint or secure-endpoint");
        if (builder.endpoint != null) {
            checkArgument(builder.endpoint.getPath().isEmpty(), "endpoint path must be empty, was: %s", builder.endpoint.getPath());
        }
        if (builder.secureEndpoint != null) {
            checkArgument(builder.secureEndpoint.getPath().isEmpty(), "secure-endpoint path must be empty, was: %s", builder.secureEndpoint.getPath());
        ...
        checkArgument(Strings.isNullOrEmpty(builder.identity) ^ !Strings.isNullOrEmpty(builder.credential), "Must provide both identity and credential");
        ...
        handler = new S3ProxyHandlerJetty(
                builder.blobStore, builder.authenticationType,
                builder.identity, builder.credential,
    ...
    public static final class Builder {
        ...
        private long v4MaxNonChunkedRequestSize = 5L * 1024 * 1024 * 1024;
        ...
        public static Builder fromProperties(Properties properties) throws URISyntaxException {
            ...
            if (endpoint == null && secureEndpoint == null) {
                throw new IllegalArgumentException("Properties file must contain: " +
                        S3ProxyConstants.PROPERTY_ENDPOINT + " or " + S3ProxyConstants.PROPERTY_SECURE_ENDPOINT);
            }
            if (endpoint != null) {
            ...
            String gatewayHost = properties.getProperty(S3ProxyConstants.PROPERTY_GATEWAY_HOST);
            if (!Strings.isNullOrEmpty(gatewayHost)) {
                builder.gatewayHost(gatewayHost);
            }

            String maxSinglePartObjectSize = properties.getProperty(S3ProxyConstants.PROPERTY_MAX_SINGLE_PART_OBJECT_SIZE);
        ...
        public Builder gatewayHost(String gatewayHost) {
            this.gatewayHost = requireNonNull(gatewayHost);
            return this;
        }
