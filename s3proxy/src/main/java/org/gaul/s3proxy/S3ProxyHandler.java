...
public class S3ProxyHandler {
    ...
    private static final String FAKE_INITIATOR_ID = "arn:aws:iam::111122223333:"
    ...
    private static final CharMatcher VALID_BUCKET = VALID_BUCKET_FIRST_CHAR
            .or(CharMatcher.is('.'))
            .or(CharMatcher.is('_'))
            .or(CharMatcher.is('-'));
    private static final long MAX_MULTIPART_COPY_SIZE = 5L * 1024L * 1024L * 1024L;
    ...
    private final Optional<String> gatewayHost;
    ...
    private final Cache<Map.Entry<String, String>, String> lastKeyToMarker =
            CacheBuilder.newBuilder().maximumSize(10000).expireAfterWrite(10, TimeUnit.MINUTES).build();
    ...
    public S3ProxyHandler(final BlobStore blobStore,
        ...
        this.gatewayHost = Optional.ofNullable(gatewayHost);
    ...
    public final void doHandle(HttpServletRequest baseRequest,
        ...
        for (String headerName : Collections.list(request.getHeaderNames())) {
            ...
            if (headerName.equalsIgnoreCase(HttpHeaders.DATE)) {
                ...
                if (!Strings.isNullOrEmpty(request.getHeader(AwsHttpHeaders.DATE))) {
                    hasXAmzDateHeader = true;
                }
            }
        }
        boolean haveBothDateHeader = hasDateHeader && hasXAmzDateHeader;
        ...
        Map.Entry<String, BlobStore> provider = blobStoreLocator.locateBlobStore(
        ...
        if (anonymousIdentity) {
            ...
            switch (authHeader.getAuthenticationType()) {
                ...
                case NONE:
                    break;
                default:
                    throw new IllegalArgumentException("Unhandled type: " + authHeader.getAuthenticationType());
            }
...
//                        is = new ChunkedInputStream(is);
                        ...
                        if (contentSha256 != null && !contentSha256
                                .equals(BaseEncoding.base16().lowerCase().encode(hash))) {
                    ...
                    expectedSignature = AwsSignature.createAuthorizationSignatureV4(baseRequest, authHeader,
                            payload, uriForSigning, gatewayHost, credential);
    ...
    private void handleGetBlob(HttpServletRequest request, HttpServletResponse response, BlobStore blobStore,
        ...
        if (range != null && range.startsWith("bytes=") && range.indexOf(',') == -1) {
        ...
        if (action != null && this.actionRepositoryMap.containsKey(containerName)) {
            ...
            if (blobContentType == null) {
                logger.info("Getting meta from S3");
                BlobMetadata blobMetadata = blobStore.blobMetadata(containerName, blobName);
                if (blobMetadata == null) {
                    throw new S3Exception(S3ErrorCode.NO_SUCH_KEY);
                }
                blobContentType = blobMetadata.getContentMetadata().getContentType();
...
//                logger.info(uri.toString());
            ...
            logger.info("No query for GET object " + blobName);
    ...
    private void handlePutBlob(HttpServletRequest request,
        ...
        for (String headerName : Collections.list(request.getHeaderNames())) {
            ...
            } else if (headerName.equalsIgnoreCase(AwsHttpHeaders.DECODED_CONTENT_LENGTH)) {
        ...
        if (contentMD5String != null) {
            try {
                contentMD5 = HashCode.fromBytes(Base64.getDecoder().decode(contentMD5String));
            } catch (IllegalArgumentException iae) {
        ...
        if (action != null && this.actionRepositoryMap.containsKey(containerName)) {
            ...
            final InputStreamEntity streamEntity = new InputStreamEntity(is, request.getContentLength(),
            ...
            if (entity != null) {
                ...
                JSONObject JSONResponse = new JSONObject(new String(data, StandardCharsets.UTF_8));
                System.out.println(new String(data, StandardCharsets.UTF_8));
                etag = (String) JSONResponse.get("etag");
            ...
            String blobStoreType = getBlobStoreType(blobStore);
            if (blobStoreType.equals("azureblob") && contentLength > 256 * 1024 * 1024) {
    ...
    private void handlePostBlob(HttpServletRequest request,
        ...
        Map.Entry<String, BlobStore> provider = blobStoreLocator.locateBlobStore(authHeader.getIdentity(),
                null, null);
        if (provider == null) {
