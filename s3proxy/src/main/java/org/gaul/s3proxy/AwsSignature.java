...
final class AwsSignature {
    ...
    private static String buildCanonicalHeaders(HttpServletRequest request,
            List<String> signedHeaders, Optional<String> virtualHost) {
        ...
        for (String header : headers) {
            ...
            if (virtualHost.isPresent() && header.equalsIgnoreCase("host")) {
                String host = virtualHost.get().trim();
                if (!host.startsWith("\"")) {
                    host = REPEATING_WHITESPACE.matcher(host).replaceAll(" ");
                }
                headersWithValues.append(host);
            } else {
    ...
    private static String createCanonicalRequest(HttpServletRequest request,
                                                 ...
                                                 String hashAlgorithm, Optional<String> virtualHost)
        ...
        String canonicalRequest = Joiner.on("\n").join(
                ...
                buildCanonicalHeaders(request, signedHeaders, virtualHost) + "\n",
    ...
    static String createAuthorizationSignatureV4(
            ...
            byte[] payload, String uri, Optional<String> virtualHost, String credential)
        ...
        String canonicalRequest = createCanonicalRequest(request, uri, payload,
                authHeader.getHashAlgorithm(), virtualHost);
