...
public final class GlobBlobStoreLocator implements BlobStoreLocator {
    ...
    public Map.Entry<String, BlobStore> locateBlobStore(String identity, String container, String blob) {
        Map.Entry<String, BlobStore> locatorEntry = locator.get(identity);
        Map.Entry<String, BlobStore> globEntry = null;
        if (container != null) {
            for (Map.Entry<PathMatcher, Map.Entry<String, BlobStore>> entry : globLocator.entrySet()) {
                if (entry.getKey().matches(FileSystems.getDefault().getPath(container))) {
        ...
        if (globEntry == null) {
            if (identity == null) {
                ...
                return Maps.immutableEntry(null,
                        globLocator.entrySet().iterator().next().getValue().getValue());
            }
            return locatorEntry;
        }
        if (identity == null) {
        ...
        return Maps.immutableEntry(locatorEntry.getKey(), globEntry.getValue());
    }
}
